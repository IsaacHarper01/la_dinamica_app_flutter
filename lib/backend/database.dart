...
//FETCH A SINGLE DATA FROM TABLE
  ...
  Future<Map<String, dynamic>?> fetchLastPayandStudentlData(int userId) async {
    final db = await _openDatabase();
    final List<Map<String, dynamic>> lastPay = await db.query(
      'Payments',
      where: 'userId = ?',
      whereArgs: [userId],
      orderBy: 'id DESC',
...
//DELETE FUNCTIONS
  ...
  Future<int> deleteRegister(int id, String? table) async {
    final db = await _openDatabase();
    if (table == 'General') {
      return db.delete('General', where: 'id=?', whereArgs: [id]);
  ...
  Future<void> deleteAttendance(int id, String date) async {
    ...
    await db.transaction((txn) async {
      int deletedAttendance = await txn.delete(
        'Attendance',
        where: 'userId = ? AND date = ?',
        whereArgs: [id, date],
      );
      int deletedPayment = await txn.delete(
        'Payments',
        where: 'userId = ? AND date = ?',
        whereArgs: [id, date],
      );
