...
class DataStoreReadService {
  ...
  Future<List<Payment>> getPaymentsRange(
    ...
    try {
      List<Payment> payments = await Amplify.DataStore.query(
        Payment.classType,
        where: Payment.DATE
            .between(TemporalDate(startDate), TemporalDate(endDate))
  ...
  Future<List<ExtraPay>> getExtraPaysRange(
    DateTime startDate,
    DateTime endDate,
  ) async {
    return Amplify.DataStore.query(
      ExtraPay.classType,
      where: ExtraPay.DATE.between(
  ...
  Future<List<ExtraPay>> getTodayExtraPays(String date) async {
    return Amplify.DataStore.query(
      ExtraPay.classType,
      where: ExtraPay.DATE.eq(date),
    );
  }
  ...
  Future<List<Sale>> getSalesPerRange(
    ...
    try {
      List<Sale> sales = await Amplify.DataStore.query(
        Sale.classType,
        where: (Sale.TENANT_ID.eq(tenantId)).and(
          Sale.DATE.between(TemporalDate(startDate), TemporalDate(endDate)),
        ),
      );
  ...
  Future<Map<String, dynamic>> getLastPayandStudentData(
    int userId,
    String tenantId,
  ) async {
    dynamic studentData;
    ...
    try {
      List<Student> general = await Amplify.DataStore.query(
        Student.classType,
        where: Student.USER_ID.eq(userId).and(Student.CLIENT_ID.eq(tenantId)),
      );
  ...
  Future<Student?> checkIfStudentExists(String uuid, String tenantId) async {
    try {
      ...
      List<Student> general = await Amplify.DataStore.query(
        Student.classType,
        where: Student.ID.eq(uuid).and(Student.CLIENT_ID.eq(tenantId)),
      );
  ...
  Future<List<List<String>>> getAgesandAddress(
    List<int> ids,
    String tenantId,
  ) async {
    try {
    ...
    try {
      List<Attendance> attendance = await Amplify.DataStore.query(
        ...
        where: Attendance.DATE
            .eq(date)
            .and(
              Attendance.CLIENT_ID.eq(tenantId).and(Attendance.STATUS.eq(true)),
            ),
      );
      return attendance;
    } catch (e) {
  ...
  Future<List<Expense>> getExpensesRange(
    Tenant tenant,
    DateTime start,
    DateTime end,
  ) async {
    try {
      final List<Expense> allexpenses = await Amplify.DataStore.query(
        Expense.classType,
        where: Expense.DATE
            .between(TemporalDate(start), TemporalDate(end))
  ...
  Future<void> verifyPayment(
    ...
    String tenantId,
    String profId,
    LocalPlan defaultPlan,
  ) async {
    try {
      if (student.remainClasses! > 0) {
        final updatedStudent = student.copyWith(
          remainClasses: student.remainClasses! - 1,
        );
    ...
    try {
      ...
      if (user != null) {
        ...
        if (access.isEmpty) {
          ...
          safePrint(
            '✅ Usuario con ID ${user.user_id} ha sido dado acceso al tenant con ID ${tenant.tenant_id} con permisos: $permissions',
          );
          return;
        } else {
          ...
          safePrint(
            '✅ Permisos del usuario ${user.user_id} han sido actualizados: $permissions',
          );
        }
      }
    } catch (e) {
  ...
  ''';
    ...
    try {
      ...
      final userAccessList =
          items
              .map(
                (item) => UserAccess.fromJson(Map<String, dynamic>.from(item)),
              )
              .toList();
  ...
  Future<List<JoinMetric>?> getJoinMetrics(
    String tenantId,
    Evaluations exam,
  ) async {
    try {
      final joinMetrics = await Amplify.DataStore.query(
        ...
        where: JoinMetric.TENANT_ID
            .eq(tenantId)
            .and(JoinMetric.EVALUATION.eq(exam.id)),
      );
      safePrint(
        '✅ Métricas conjuntas obtenidas correctamente ${joinMetrics.first.metric!.name}',
      );
      return joinMetrics;
    } catch (e) {
  ...
  Future<List<StudentExamResults>> getStudentResultsRange(
    ...
    do {
      ...
      allItems.addAll(items.map((e) => StudentExamResults.fromJson(e)));
  ...
  Future<List<StudentExamResults>> getLastStudentExamResult(
    ...
    // 🔥 filter + sort locally
    final filtered = items.where((e) => e['tenant_id'] == tenantId).toList();
  ...
  Future<List<StudentExamResults>> getAllStudentResults(
    String tenaniId,
    String studentId,
  ) async {
    final results = await Amplify.DataStore.query(
      StudentExamResults.classType,
      where: StudentExamResults.STUDENT
          .eq(studentId)
  ...
  Future<Product?> productExists(String productCode, String tenaniId) async {
    ...
    try {
      final products = await Amplify.DataStore.query(
        Product.classType,
        where: Product.TENANT_ID.eq(tenaniId).and(Product.CODE.eq(productCode)),
      );
  ...
  Future<void> sellProduct(
    ...
    try {
      ...
      if (quantity > oldStock) {
        throw StateError('No hay suficientes unidades disponibles');
      }

      final newProduct = product.copyWith(stock: oldStock - quantity);
  ...
  Future<String> apiAssistant() async {
    ...
    if (session is CognitoAuthSession) {
      ...
      final response = await http.post(
        Uri.parse(
          "https://k424jq6fj1.execute-api.us-east-1.amazonaws.com/laDinamicaApp/generate",
        ),
        headers: {
  ...
  Future<String> generateRutineByGroup(String description) async {
    ...
    if (session is CognitoAuthSession) {
      ...
      final response = await http.post(
        Uri.parse(
          "https://k424jq6fj1.execute-api.us-east-1.amazonaws.com/laDinamicaApp/generateByGroup",
        ),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $accessToken',
